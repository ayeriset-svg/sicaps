...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
    @foreach($modules as $mod)
        ...
        @if(! $needsWork)
            ...
            <a href="{{ route('logbook.show', $mod) }}" class="block rounded-2xl shadow-sm border p-5 hover:shadow-md transition {{ $isAsmt ? 'border-pink-200 bg-pink-50' : 'border-rose-100 bg-white' }}">
                <div class="flex items-center justify-between mb-2 gap-2">
                    <span class="text-xs font-medium {{ $isAsmt ? 'text-pink-500' : 'text-slate-400' }}">{{ $mod->week_label }} · {{ $mod->code }}</span>
                    @if($isAsmt)<span class="text-xs rounded-full bg-pink-100 text-pink-700 px-2 py-0.5 font-medium">Assessment</span>@else<span class="text-xs rounded-full bg-slate-100 text-slate-500 px-2 py-0.5 font-medium">📄 Materi saja</span>@endif
                </div>
                <h3 class="font-semibold {{ $isAsmt ? 'text-pink-900' : 'text-slate-800' }} leading-snug">{{ $mod->title }}</h3>
                <p class="text-sm {{ $isAsmt ? 'text-pink-700' : 'text-slate-500' }} mt-1">{{ \Illuminate\Support\Str::limit(trim(html_entity_decode(strip_tags($mod->description ?? ''))), 80) }}</p>
        ...
        @else
            ...
            <a href="{{ route('logbook.show', $mod) }}" class="block rounded-2xl shadow-sm border p-5 hover:shadow-md transition {{ $isAsmt ? 'border-pink-200 bg-pink-50' : ($mod->isIndividual() ? 'border-orange-200 bg-orange-50' : 'border-rose-100 bg-white') }}">
                <div class="flex items-center justify-between mb-2 gap-2">
