...
<div class="mb-6 flex items-start justify-between flex-wrap gap-3">
    ...
    @if(auth()->user()->isSuperadmin())
        <a href="{{ route('admin.manual-books.index') }}" class="rounded-lg border border-rose-200 text-brand px-4 py-2 text-sm hover:bg-rose-50">⚙️ Kelola Manual Book</a>
    @endif
</div>
...
@forelse($books as $book)
    @if($loop->first)<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">@endif
        <a href="{{ route('manual.show', $book) }}" class="block bg-white rounded-2xl shadow-sm border border-rose-100 p-5 hover:shadow-md transition">
            ...
            @if($book->file_path)<span class="inline-block mt-2 text-xs rounded-full bg-rose-50 text-brand px-2 py-0.5">📎 Ada lampiran</span>@endif
