...
* **Review & Feedback (FR-3.3):** dilakukan **Superadmin** (menggantikan peran dosen) — memberi status + feedback per modul/tugas. Superadmin juga dapat **mengedit isi logbook** (koreksi payload field richtext/link/berkas; disanitasi anti-XSS, status review tidak berubah) via tombol "Edit Isi". PDF logbook/tugas menyertakan **Riwayat Revisi** (snapshot tiap submit/review: versi, status, catatan, oleh, tanggal).
