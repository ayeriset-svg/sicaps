...
* **Superadmin (Koordinator Capstone):** kontrol penuh — Master Data (User, Mahasiswa, Mitra, Topik), Manajemen Tahun Ajaran, **Kelola Modul dinamis**, review & feedback logbook, input nilai assessment, buka/tutup Peer 180° per tahap, pengaturan bobot & penalti, presensi, rekap & override nilai, Summary Report, Audit Log.
* **Autentikasi:** login via NIM/email; aktivasi = wajib ganti sandi saat login pertama. **Lupa kata sandi:** tautan pada halaman login mengarahkan mahasiswa ke **WhatsApp koordinator** (reset dilakukan koordinator via Master Mahasiswa) — tidak ada reset mandiri via email.
...
| Capaian (PLO/CLO/Sub-CLO) | Lihat di modul | Lihat di modul | Kelola (CRUD) |
...
* **Master User & Master Mahasiswa:** CRUD penuh + **import Excel/CSV**. **Master Mahasiswa** kini punya aksi **Tambah / Edit / Hapus per baris** + **hapus massal** (pilih banyak via checkbox → Hapus Terpilih; yang masih tergabung tim dilewati) untuk memperbaiki/membersihkan data keliru hasil import — lengkap dengan **kelas & angkatan** untuk pemantauan per kelas, **tanpa kolom email** (identitas cukup NIM; email dibuat otomatis internal untuk autentikasi). Edit dapat mereset sandi (→ wajib ganti saat login). **Hapus diblokir** bila mahasiswa masih memimpin/tergabung dalam tim (mencegah terhapusnya data tim akibat cascade). Import mendukung **data + nilai historis** (kolom `year,semester,final_score,grade_letter` opsional → membuat tahun ajaran arsip + final grade).
* **Template Import (FR-1.3):** setiap fitur import menyediakan **unduhan template Excel (.xlsx)** berisi header + baris contoh — untuk Master Mahasiswa dan Master User (endpoint `admin.templates.students`, `admin.templates.users`). Berkas `.xlsx` dibuat tanpa dependency eksternal (helper `App\Support\Xlsx` berbasis ZipArchive). **Importer menerima `.xlsx` maupun `.csv`** dan tahan-banting: mengabaikan BOM UTF-8, mendeteksi pemisah (`,` / `;` / tab — Excel ID sering `;`), serta membaca shared-string & inline-string XLSX (parsing via `App\Http\Controllers\Concerns\ReadsCsv` + `App\Support\Xlsx`). NIM ditulis sebagai teks agar tidak berubah menjadi notasi ilmiah.
...
* **Manajemen Tim (FR-1.5):** superadmin memantau seluruh tim (filter kelas, tandai HKI) serta **mengedit** (nama tim, ranah usaha, ganti ketua — ketua wajib salah satu anggota) dan **menghapus** tim. Menghapus tim ikut menghapus seluruh data terkait (anggota, logbook/tugas, nilai, presensi tim) via cascade — dengan konfirmasi.
* **Capaian Pembelajaran / CLO (FR-1.6):** superadmin mengelola **PLO → CLO → Sub-CLO** (menu *Capaian (CLO)*) per **tahun ajaran** (bisa berbeda tiap tahun). Sub-CLO **dipetakan ke tiap modul praktikum** (multi-pilih di Kelola Modul) untuk pengukuran capaian, dan **ditampilkan** pada modul (form admin, preview/PDF, serta sisi mahasiswa).
* **Konsistensi Tahun Ajaran (FR-1.7):** seluruh data yang bersifat tahunan — **modul, CLO/PLO, topik, mitra, tim, nilai, presensi** — disimpan & ditampilkan mengikuti **tahun ajaran aktif** yang dipilih (query terikat `academic_year_id` aktif). *Master Mahasiswa* tetap master global lintas angkatan (identitas by NIM).
...
* **Kelola Topik (FR-2.2):** katalog topik yang **ditawarkan** superadmin (mitra, judul, **fitur umum**, **fitur AI**, deskripsi). Katalog dapat ditandai tersedia/tidak. **Satu topik katalog boleh dipilih lebih dari satu kelompok**, dan tiap kartu katalog menampilkan **jumlah kelompok** yang mengajukannya.
* **Pemilihan & Edit Topik oleh Tim (Modul 1):** ketua tim boleh **memilih dari katalog** ATAU **mengajukan topik mandiri** (opsional bermitra). Ketua dapat **mengedit** topiknya kapan saja — fitur (umum/AI), mitra & judul (untuk mandiri), serta **jenis/bidang perusahaan** (Jasa/Dagang/Manufaktur); **menyimpan perubahan otomatis mengajukan ulang** (status → pending) untuk ditinjau, meski sebelumnya sudah disetujui. Superadmin melakukan **approve/reject** dengan catatan.

### EPIC 3 — Dynamic Digital Logbook & Module Manager
* **Kelola Modul (FR-3.1):** superadmin mengatur jumlah modul, urutan, judul, jenis (module/assessment/other), **materi** (Tujuan, Alat & Bahan, Referensi, Deskripsi/Materi Teori, Tugas — *field "Aturan Penggunaan AI" dihapus karena sudah diwakili Level Batasan AI*), dan **daftar field** logbook (label, tipe **richtext/link/file**, wajib/opsional). Tipe `file` mengaktifkan **unggah dokumen PDF/Word** oleh mahasiswa. Tiap modul/tugas juga punya penanda **Buka/Tutup** (`is_open`), **Membutuhkan pengerjaan** (`requires_submission` — bila off, modul hanya materi & form isian TIDAK muncul di sisi mahasiswa), **Tugas Individu** (`is_individual`) beserta slot presensi opsional, serta **Jadwal** `opens_at`/`closes_at` (kapan dibuka & deadline). Modul juga dapat memetakan **Sub-CLO** (capaian) dan menyediakan **Preview dokumen + Generate PDF** (materi + field, siap cetak seperti sisi mahasiswa).
* **Digital Logbook (FR-3.2):** form logbook di-render dinamis sesuai definisi field modul. Ketua tim submit; status `Not Started → Pending → (Revision Needed | Approved | Rejected)`; versioning payload otomatis pada tiap submit ulang. **Setelah `Approved` (PASS) atau `Rejected`, logbook TERKUNCI** — editor tidak ditampilkan ke mahasiswa & submit ditolak server. **Riwayat revisi** mencatat status FINAL tiap keputusan review (mis. langsung "Disetujui/Done" beserta waktu & komentar koordinator, bukan lagi "Pending") — termasuk pada PDF. **Ada/tidaknya form pengerjaan ditentukan oleh flag "Membutuhkan pengerjaan" — BUKAN tipe modul.** Modul/tugas/**assessment** apa pun akan menampilkan **form isian** ke mahasiswa bila flag ini aktif (mis. assessment yang diberi field oleh koordinator); bila nonaktif, modul hanya menampilkan **materi** (tetap dapat dibuka mahasiswa). Gating buka/tutup & jadwal juga seragam untuk semua tipe.
...
* **Tugas Individu / Assignment (FR-3.7):** selain logbook tim, superadmin dapat menandai modul sebagai **Tugas Individu** (`is_individual`). Fitur & tools sama persis dengan logbook (field dinamis, richtext/link/file, cetak PDF, cek AI, proofreader, riwayat revisi) — **bedanya hanya model pengerjaan**: logbook tim boleh **diwakilkan ketua**, sedangkan tugas **wajib dikerjakan tiap mahasiswa secara individu** (submission per-user; `module_logbooks.user_id`). Bila mahasiswa submit tugas lalu superadmin men-**PASS**, mahasiswa **otomatis ditandai HADIR** pada slot presensi (Minggu/Sesi) yang diset di modul; bila status dibatalkan dari PASS, penanda hadir otomatis tersebut dicabut. **Opsi "Dihitung sebagai presensi kelas" (`counts_as_attendance`):** dikumpulkan tepat waktu & **PASS** → **HADIR**; **Rejected/ditolak** → **ALPA**. Mahasiswa yang **tidak mengumpulkan hingga deadline otomatis ALPA** — difinalisasi otomatis (lazy) saat koordinator membuka menu Presensi / merekalkulasi nilai (tanpa cron), dan tetap tersedia tombol manual **"Proses Presensi"**.
* **Review & Feedback (FR-3.3):** dilakukan **Superadmin** (menggantikan peran dosen) — memberi status + feedback per modul/tugas. Antrian review memiliki **2 tab** (**Perlu Review** = Pending; **Sudah Direview** = Approved/Revision Needed) + **filter Modul, Kelas, Status**. Superadmin juga dapat **mengedit isi logbook** (koreksi payload field richtext/link/berkas; disanitasi anti-XSS, status review tidak berubah) via tombol "Edit Isi". PDF logbook/tugas menyertakan **Riwayat Revisi** (snapshot tiap submit/review: versi, status, catatan, oleh, tanggal) **serta Skor Tata Tulis** (proofreader). Skor tata tulis juga tampil di halaman mahasiswa.
* **Audit Log (FR-3.10):** menu **Audit Log** (superadmin) mencatat aktivitas per user — login/logout & seluruh perubahan data (POST/PUT/DELETE via middleware `LogActivity`) dengan waktu, user, aksi, rute, URL, & IP; dapat difilter per user/aksi/kata kunci.
* **Cek Kemiripan Jawaban (FR-3.11):** untuk **tugas individu**, tombol di halaman Kelola Modul menjalankan pengecekan **kemiripan jawaban antar mahasiswa** (Jaccard shingle 3-kata; 100% = identik). Hasil muncul di **Review Logbook**: bila kemiripan ≥ ambang, tampil **peringatan merah** di atas panel Periksa AI (daftar mahasiswa mirip + persentase).
...
* **FR-4.3 Input & Edit Nilai:** superadmin menginput nilai rubrik per tim; nilai yang sudah tersimpan **dapat diedit** (form ter-prefill nilai terakhir + badge "sudah dinilai"), dan **dikosongkan** untuk menghapus nilai keliru. Perubahan memicu kalkulasi ulang NA (FR-5.3).
...
* **FR-5.2 Penalti Kehadiran (berbasis HARI tidak hadir, configurable):** presensi 16 minggu × 2 sesi; "hari tidak hadir" = jumlah sesi berstatus *alpa*. Filter menu Presensi **bertingkat**: memilih **Kelas** menyaring opsi **Kelompok** hanya untuk kelas tersebut.
* **FR-5.5 Rekap Presensi Mahasiswa:** dashboard mahasiswa menampilkan **rekap kehadiran** (Hadir/Izin/Sakit/Alpa) beserta **persentase total kehadiran**, dengan peringatan bila alpa berisiko nilai E.
...
* **FR-5.4 Rekap Nilai & Export:** halaman Rekap Nilai menampilkan **nilai tiap stage (A1/A2/A3)** di samping NA, hari alpa, penalti, nilai akhir, indeks, & override — nilai stage diambil dari `breakdown_json`. Tersedia **Export Excel (.xlsx)** rekap (NIM, Nama, Kelas, A1/A2/A3, NA, Alpa, Penalti, Akhir, Indeks), memakai helper `App\Support\Xlsx` tanpa dependency.
...
| `modules` | Modul dinamis per tahun ajaran (order, week_label, code, type, assessment_stage, materi, fields_json); **`is_open`**, **`opens_at`/`closes_at`** (jadwal), **`requires_submission`** (butuh pengerjaan), **`is_individual`**, **`attendance_week`/`attendance_session`** + **`counts_as_attendance`** + **`attendance_finalized_at`** (tugas = presensi kelas). |
| `plos` / `clos` / `sub_clos` | Capaian per tahun ajaran: PLO → CLO (→ PLO) → Sub-CLO (→ CLO). code, description, order_index. |
| `module_sub_clo` | Pivot modul ↔ Sub-CLO (pemetaan capaian per modul). |
| `module_logbooks` | payload_json per field, status_approval (Not Started/Pending/Revision Needed/Approved/**Rejected**), feedback, submitted_at; **`user_id`** (NULL = logbook tim; terisi = tugas individu) — unique (team, module, user); indikasi AI (`ai_*`); tata tulis (`proofread_*`); **kemiripan** (`similarity_max`, `similarity_json`, `similarity_checked_at`). |
| `activity_logs` | Audit aktivitas: user_id, action, description, method, route, url, ip, timestamps. |
