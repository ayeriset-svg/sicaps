...
* **Kelola Modul (FR-3.1):** superadmin mengatur jumlah modul, urutan, judul, deskripsi/materi, jenis (module/assessment/other), dan **daftar field** logbook (label, tipe **richtext/link/file**, wajib/opsional). Tipe `file` mengaktifkan **unggah dokumen PDF/Word** oleh mahasiswa.
* **Digital Logbook (FR-3.2):** form logbook di-render dinamis sesuai definisi field modul. Ketua tim submit; status `Not Started → Pending → (Revision Needed | Approved)`; versioning payload otomatis pada tiap submit ulang.
* **Review & Feedback (FR-3.3):** dilakukan **Superadmin** (menggantikan peran dosen) — memberi status + feedback per modul.
...
| `modules` | Modul dinamis per tahun ajaran (order, week_label, code, type, assessment_stage, description, fields_json). |
| `module_logbooks` | payload_json per field, status_approval, feedback, submitted_at (unique team+module); indikasi AI (`ai_percentage`, `ai_text_percentage`, `ai_image_percentage`, `ai_detail_json`, `ai_checked_at`); tata tulis (`proofread_score`, `proofread_json`, `proofread_checked_at`). |
