...
| `modules` | Modul dinamis per tahun ajaran (order, week_label, code, type, assessment_stage, materi, fields_json); **`is_open`**, **`opens_at`/`closes_at`** (jadwal), **`requires_submission`** (butuh pengerjaan), **`is_individual`**, **`attendance_week`/`attendance_session`** + **`counts_as_attendance`** (tugas = presensi kelas). |
